...
from module_voicecontrol import process_movement_command
...
def utterance_callback(message):
    ...
    Now includes movement command processing.
    ...
    try:
        ...
        # First try to process as a movement command
        success, movement_response = process_movement_command(user_text)
        if success:
            print(f"TARS: {movement_response}")
            generate_tts_audio(movement_response, CONFIG['TTS']['ttsoption'], 
        ...
        # If not a movement command, process normally through LLM
        reply = process_completion(user_text)
        print(f"TARS: {reply}")
        generate_tts_audio(reply, CONFIG['TTS']['ttsoption'],
                          CONFIG['TTS']['azure_api_key'],
                          CONFIG['TTS']['azure_region'],
                          CONFIG['TTS']['ttsurl'],
                          CONFIG['TTS']['toggle_charvoice'],
